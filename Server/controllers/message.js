...
module.exports = {
			...
			async (err, result) => {
				if (result.length > 0) {
					...
					await Message.update(
						...
						.then(() => {
							res.status(HttpStatus.OK).json({ message: 'Message Sent Again' });
						})
						.catch(err => {
							res
								.status(HttpStatus.INTERNAL_SERVER_ERROR)
								.json({ message: 'Error sending msg again' });
						});
				} else {
					...
					await newMessage
						...
						.catch(err => {
							res
								.status(HttpStatus.INTERNAL_SERVER_ERROR)
								.json({ message: 'Error sending msg' });
						});
				}
			}
		);
	...
	async MarkReceiverMessages(req, res) {
		...
		const msg = await Message.aggregate([
			{ $unwind: '$message' }, //Destructure Array
			{
				$match: {
					$and: [
		...
		if (msg.length > 0) {
			try {
				...
				res.status(HttpStatus.OK).json({ message: 'Messages read' });
			} catch (err) {
				res
					.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.json({ message: 'Messages read error' });
			}
		}
	},
	...
	async MarkAllMessages(req, res) {
		...
		if (msg.length > 0) {
			try {
				...
				res.status(HttpStatus.OK).json({ message: 'All Messages read' });
			} catch (err) {
				res
					.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.json({ message: 'All Messages read error' });
			}
		}
	}
};
